...
| `stopRescueAttempts` | int | 3 | Stillborn/short-decode rescue budget. Some (reference clip, text) pairs decode to pure silence (stop head saturated) or an implausibly short take — deterministically (greedy decode), at ANY threshold. Each rescue re-encodes the reference on a log-spaced gain ladder (−1/−3/−6 dB — basin escape needs SPACING; memoized) and a take is accepted only if it clears ~0.03 s/char; streaming holds chunks ≤ ~2.5 s so a rejected attempt emits nothing. Budget exhausted ⇒ longest take. 0 disables. |
